...
import { getExpense, updateExpense } from '../../../src/services/expenses.service';
...
export default function EditExpenseScreen() {
  ...
  useEffect(() => {
    ...
    getExpense(expenseId).then((e) => {
      ...
      try { setDate(new Date(e?.date ?? Date.now())); } catch { setDate(new Date()); }
      setCategory((e?.category as ExpenseCategory) ?? 'GasolinaDiesel');
      setCustomCategory(e?.customCategory ?? '');
      setAmount(String(e?.amount ?? ''));
      if (e?.receiptFileId) {
        setFileId(e.receiptFileId);
        setFileName(e?.receiptFileName ?? 'receipt');
        setFileType(e?.receiptContentType ?? '');
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'Failed to update expense';
      setError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          <View style={styles.card}>
            ...
            <Text style={[styles.label, { marginTop: spacing.xs }]}>Receipt</Text>
            {fileUri || (fileId && fileName) ? (
              <View style={styles.filePreview}>
                {(fileType?.startsWith('image/') && fileUri) ? (
          ...
          <GradientButton title="Save Changes" onPress={handleSave} loading={loading} disabled={uploading} style={{ marginTop: spacing.lg }} />
        </ScrollView>
