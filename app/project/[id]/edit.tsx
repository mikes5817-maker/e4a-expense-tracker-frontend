...
import { getProject, updateProject } from '../../../src/services/projects.service';
...
export default function EditProjectScreen() {
  ...
  useEffect(() => {
    ...
    getProject(id).then((p) => {
      ...
      setName(p?.name ?? '');
      try { setDate(new Date(p?.date ?? Date.now())); } catch { setDate(new Date()); }
    }).catch(() => {});
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await updateProject(id, { projectNumber: projectNumber.trim(), name: name.trim(), date: date.toISOString() });
      router.back();
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'Failed to update project';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <GradientButton title="Save Changes" onPress={handleSave} loading={loading} style={{ marginTop: spacing.lg }} />
        </ScrollView>
